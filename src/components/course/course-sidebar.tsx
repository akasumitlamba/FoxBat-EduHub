...
export function CourseSidebar({ course, activeLesson, setActiveLesson }: CourseSidebarProps) {
  const { isLessonCompleted, progressPercentage, isInitialized } = useCourseProgress(course.id);
  ...
  }, [isInitialized, progressPercentage, totalLessons, isLessonCompleted, activeLesson]);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="p-4">
        ...
        <p className="text-xs text-muted-foreground mt-1">{Math.round(currentProgress)}% complete</p>
      </div>
      <ScrollArea className="flex-1">
        <Accordion type="multiple" defaultValue={course.modules.map(m => m.id)} className="w-full px-4">
          {course.modules.map((module: Module) => (
            <AccordionItem value={module.id} key={module.id}>
              <AccordionTrigger className="font-semibold text-left">{module.title}</AccordionTrigger>
              <AccordionContent>
